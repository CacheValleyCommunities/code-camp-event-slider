...
export default class WebSocketController {
    constructor(scene) {
        ...
        this.heartbeatInterval = null;        // Get WebSocket URL - always use the proxy path for zero-config connectivity
        const wsProtocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
        const wsHost = window.location.host; // Use current host:port
        this.serverUrl = `${wsProtocol}//${wsHost}/websocket/`;

        console.log(`📡 Using WebSocket proxy URL: ${this.serverUrl} (from ${window.location.origin})`);
